...
const quizQuestions = []; // Define an empty array to store the quiz questions
...
function generateQuiz(numQuestions) {
    ...
    // Shuffle the hiragana list
    const shuffledHiragana = shuffleArray(hiraganaList);

    // Create the quiz questions
    quizQuestions.length = 0; // Reset the quizQuestions array
    for (let i = 0; i < numQuestions; i++) {
        ...
        // Store the question and correct answer in the quizQuestions array
        quizQuestions.push({ hiragana, correctRomaji: romaji });
    }

    quizForm.style.display = "block"; // Display the quiz section
    document.getElementById("quiz-title").textContent = quizTitle;

    return quizQuestions; // Return the quizQuestions array
}
...
function validateQuiz(event) {
    ...
    for (let i = 0; i < quizInputs.length; i++) {
        ...
        // Check if the question input and quizQuestions entry exist
